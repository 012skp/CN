...
while(1){
	...
	struct child *check_child;
	check_child = children;
	FD_ZERO(&fds);
	int maxfd = 0;
	int no_of_clients =0;
	while(check_child != NULL){
		FD_SET(check_child->fd[0],&fds);
		if(check_child->fd[0] > maxfd ) maxfd = check_child->fd[0];
		check_child = check_child->next_child;
		no_of_clients++;
	}
	if(no_of_clients > 0) if(select(maxfd+1,&fds,NULL,NULL,&tv)) children = data_processing(children);
	else usleep(1000000/no_of_clients);
}
